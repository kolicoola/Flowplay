...
const getAppParamValue = (paramName, { defaultValue = undefined, removeFromUrl = false } = {}) => {
	...
	const normalizedStorageKey = `payflow_${toSnakeCase(paramName)}`;
	...
	if (searchParam) {
		storage.setItem(normalizedStorageKey, searchParam);
		return searchParam;
	}
	if (defaultValue) {
		storage.setItem(normalizedStorageKey, defaultValue);
		return defaultValue;
	}
	const storedValue = storage.getItem(normalizedStorageKey);
	if (storedValue) {
		return storedValue;
	}
	return null;
...
const getAppParams = () => {
	if (getAppParamValue("clear_access_token") === 'true') {
		storage.removeItem('payflow_access_token');
